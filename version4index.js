...
var outfile4menu = "undefined content";
  ...
  fs.readFile(vFileName1, 'utf8', function readFileCallback(err, data){
      if (err){
          console.log(err);
      } else {
    ...
    fs.readFile(vFileName2, 'utf8', function readFileCallback(err, data){
        if (err){
            console.log(err);
        } else {
...
setTimeout(function () {
  b4c.save_file(vFileName1, outfile);
  b4c.save_file(vFileName2, outfile4menu);
},2000);
